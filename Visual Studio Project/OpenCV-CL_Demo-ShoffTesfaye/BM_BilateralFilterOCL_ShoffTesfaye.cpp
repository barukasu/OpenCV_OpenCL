...
//	Date:		4/12/2014
//
//	Purpose:	This file contains the Bilateral Filter OCL benchmark test function
//
...
int bilateralOCL_ShoffTesfaye()
{
	...
	namedWindow("OpenCL Demo: Bilateral Filter", 1);  // Create video display window.
    ...
    for (;;)  // Infinite read video loop.
    {
        ...
        if (OPENOCL)
        {
            ocl::oclMat oclFrameSrc(frame);  // Convert frame to OCL frame for operation.
            ocl::oclMat oclFrameDest;

			/********** Actual operations w/ time calc **********/
			opTimePre = cv::getTickCount();
			ocl::bilateralFilter(oclFrameSrc, oclFrameDest, -1, 50, 7, 1);
            ...
            imshow("OpenCL Demo: Bilateral Filter", frameDest);  // Show image.
        }
        else
        {
            Mat outFrame;
			...
			/********** Actual operations w/ time calc **********/
			opTimePre = cv::getTickCount();
			bilateralFilter(frame, outFrame, -1, 50, 7, 1);
			opTimePost = cv::getTickCount();
            ...
            imshow("OpenCL Demo: Bilateral Filter", outFrame);  // Show image.
        }
