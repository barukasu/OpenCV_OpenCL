...
//	Date:		4/12/2014
//
//	Purpose:	This file contains the Canny OCL benchmark test function
//
...
int cannyOCL_ShoffTesfaye()
{
	...
	namedWindow("OpenCL Demo: Canny", 1);  // Create video display window.
    ...
    for (;;)  // Infinite read video loop.
    {
        ...
        if (OPENOCL)
        {
            ...
            ocl::Canny(oclFrameSrc, oclFrameDest, 0,30,3);
            ...
            imshow("OpenCL Demo: Canny", frameDest);  // Show image.
        }
        else
        {
            Mat outFrame;
			...
			/********** Actual operations w/ time calc **********/
			opTimePre = cv::getTickCount();
            Canny(grayFrame, outFrame, 0, 30, 3);
			opTimePost = cv::getTickCount();
            ...
            imshow("OpenCL Demo: Canny", outFrame);  // Show image.
        }
